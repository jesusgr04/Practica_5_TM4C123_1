...
extern void Configura_Reg_PWM1(uint16_t freq)
{
   ...
   //  frec, tarjeta / frec PWM = cuentas 
    ...
    GPIOB->PCTL |= (GPIOB->PCTL&0x0FF0FFFF) | 0x40040000;
    ...
    PWM0->_0_CTL = (0<<0); 
    PWM0->_1_CTL = (0<<0); 
    PWM0->_2_CTL = (0<<0); //se le asigna 0 a 0 porque es el bit para indicar lo habilitado o desabilitado según pag. 1266-
    ...
    PWM0->_2_GENA = 0x0000008C; //se usa el comparador A que es con el que trabaja el PWM2 según pag. 1248
    ...
    PWM0->_0_CMPB = 30000; 
    PWM0->_1_CMPA = 300; 
    PWM0->_2_CMPA = 2000; //Para tener el 25% en el duty cicle %75 de las 5000 cuentas

    PWM0->_0_CTL = (1<<0);// habiitar generador 1
    PWM0->_1_CTL = (1<<0);// habiitar generador 1
    PWM0->_2_CTL = (1<<0);// habiitar generador 1

    PWM0->ENABLE = (1<<1)|(1<<2)|(1<<4); // habilitar el PWM que se usa PWM2 pag. 1247 y 1233
}
